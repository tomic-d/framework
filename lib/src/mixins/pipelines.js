...
const OneTypePipelines =
{
    ...
    PipelineAdd(name, config = {}, then = null, callback = true)
    {
        ...
        if(callback)
        {
            this.pipelines.callbacks.add.forEach(callback =>
            {
                try
                {
                    callback(pipeline);
    ...
    PipelineRemove(name, callback = true)
    {
        ...
        if(callback)
        {
            this.pipelines.callbacks.remove.forEach(callback =>
            {
                try
                {
                    callback(pipeline);
    ...
    PipelineOn(name, callback)
    {
        if(!(name in this.pipelines.callbacks))
        {
            return this.Error(400, 'Pipeline catcher not found.', {pipeline: name});
        }

        this.pipelines.callbacks[name].push(callback);
    }
};
