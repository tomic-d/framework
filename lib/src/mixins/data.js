const OneTypeData =
{
    ...
    DataDescribe(config)
    {
        return Object.entries(config && typeof config === 'object' ? config : {}).map(([name, define]) =>
        {
            const entry = {
                name,
                type: define.type ? define.type : 'string',
                required: !!define.required,
                description: define.description ? define.description : ''
            };

            if (define.value !== undefined && typeof define.value !== 'object')
            {
                entry.value = define.value;
            }

            if (Array.isArray(define.options) && define.options.length)
            {
                entry.options = define.options;
            }

            const nested = define.config ? define.config : (define.each && define.each.config ? define.each.config : null);

            if (nested && typeof nested === 'object')
            {
                entry.items = this.DataDescribe(nested);
            }
