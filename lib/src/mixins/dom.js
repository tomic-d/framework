const OneTypeDOM =
{
    ...
    DOMPatch(current, target)
    {
        ...
        const shouldReplace = current.nodeType !== target.nodeType || (current.nodeType === Node.ELEMENT_NODE && current.nodeName.toUpperCase() !== target.nodeName.toUpperCase()) || target.__otExternal;
    ...
    DOMPatchChildren(current, target)
    {
        ...
        const getKey = (node) => node.nodeType === Node.ELEMENT_NODE && node.getAttribute('ot-key');
