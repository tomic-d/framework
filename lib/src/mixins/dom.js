const OneTypeDOM =
{
	...
	DOMExternal(current, target)
	{
		if(!current.__otExternal || !target.__otExternal)
		{
			if(current.__otExternal || target.__otExternal)
			{
				console.log('[DOMExternal MISS]', 'current:', current.__otExternal?.name, 'target:', target.__otExternal?.name, 'target tag:', target.tagName, 'target outerHTML:', target.outerHTML?.substring(0, 150));
			}
			return false;
		}
		...
		{
			console.log('[DOMExternal MISMATCH]', a.name, 'vs', b.name, 'key:', a.key, 'vs', b.key);
			return false;
		}

		if(a.render && b.data)
		{
			if(a.name.includes('form-input') || a.name.includes('form-textarea'))
			{
				console.log('[DOMExternal UPDATE]', a.name, 'old value:', a.render.Data.value, 'new value:', b.data.value);
			}
