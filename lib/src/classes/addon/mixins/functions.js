const AddonFunctions =
{
    ...
    FnRun(name, ...data)
    {
        ...
        if (!fn)
        {
            this.onetype.Error(404, 'Function ":name:" not found on addon ":addon:". Available: :available:.', {
                name,
                addon: this.GetName(),
                available: Object.keys(this.functions.data).join(', ') || 'none'
            });
            return null;
        }
    ...
    Fn(name, ...data)
    {
        if(!this.FnGet(name) && data.length === 1 && typeof data[0] === 'function')
        {
