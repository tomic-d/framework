const PipelineRun =
{
    ...
    _RunResolveConfig()
    {
        ...
        if(typeof this.config.in === 'string')
        {
            this.config.in = this.onetype.DataConfig(this.config.in);
        }

        if(typeof this.config.out === 'string')
        {
            this.config.out = this.onetype.DataConfig(this.config.out);
        }

        for(const join of Object.values(this.joins.data))
        {
            if(typeof join.in === 'string')
            {
                join.in = this.onetype.DataConfig(join.in);
            }

            if(typeof join.out === 'string')
            {
                join.out = this.onetype.DataConfig(join.out);
            }
        }
    ...
    _RunValidate(properties, config, join, code, label)
    {
        ...
        {
            const path = Array.isArray(error.path) && error.path.length ? error.path.join('.') : null;
            const detail = path ? (path + ': ' + error.message) : error.message;
            const where = join ? (' at join "' + join + '"') : '';

            return {
                data:    error.message,
                message: 'Pipeline "' + this.name + '" ' + label + where + ' is invalid — ' + detail,
                code,
                join
            };
        }
    ...
    _RunFinish(state, properties, context)
    {
        ...
        else if(this.config.out && Object.keys(this.config.out).length > 0)
        {
            data = {};

            for(const key of Object.keys(this.config.out))
            {
