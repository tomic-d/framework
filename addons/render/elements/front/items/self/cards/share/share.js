onetype.AddonReady('elements', (elements) =>
{
	elements.ItemAdd({
		...
		icon: 'share',
		name: 'Share Card',
		description: 'Share toolbar card with copy, native share, bookmark, like and social platforms.',
		category: 'Cards',
		author: 'OneType',
		config: {
			title: {
			...
			variant: {
				...
				options: ['bg-1', 'bg-2', 'bg-3', 'bg-4', 'border', 'size-s', 'size-m', 'size-l', 'compact', 'inline']
			},
			_copy: {
				type: 'function'
			},
		...
		render: function()
		{
			...
			this.hasCopy = this.showAction('copy');
			this.hasShare = this.showAction('share');
			this.hasBookmark = this.showAction('bookmark');
			this.hasLike = this.showAction('like');
			this.hasPlatforms = this.platforms && this.platforms.length > 0;
			this.hasCounts = this.likes != null || this.comments != null || this.shares != null;
			...
			{
				const meta = this.platformMeta[key] || {};
				...
				return {
					...
					svg: meta.svg || '',
					icon: meta.icon || '',
					href: this.buildPlatformHref(key)
				};
			});
			...
			return /* html */ `
				<div :class="'holder ' + orientation + ' ' + variant.join(' ')">
					...
					<div ot-if="hasPlatforms" class="platforms">
						...
						>
							<span ot-if="platform.svg" class="brand" ot-html="platform.svg"></span>
