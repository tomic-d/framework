...
database.Fn('crud.delete', async function(item, {connection = 'primary'} = {})
{
	...
	return knex.transaction(async (transaction) =>
	{
		...
		if(versions)
		{
			before = database.Fn('cast', item.addon, knex, await transaction(table).where('id', item.Get('id')).first());

			await transaction(table).where('id', item.Get('id')).update({
				[versions.delete]: stamp()
			});
		}
		else
		{
