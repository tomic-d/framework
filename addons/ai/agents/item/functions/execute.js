...
agents.Fn('item.execute', async function(item, payload, stream = null)
{
	const provider = item.Get('provider') ? providers.ItemGet(item.Get('provider')) : providers.Fn('default');
